...
void LoadMesh(std::vector<unsigned int> vertexIndices,std::vector<double> x,std::vector<double> y,std::vector<double> z ){
  ...
  for(int i=0;i<vertexIndices.size()-3;i+=3)
  {
      ...
      int idx2=i+1;
      int idx3=i+2;
      Point_3 p1(x.at(idx1),y.at(idx1),z.at(idx1));
      Point_3 p2(x.at(idx2),y.at(idx2),z.at(idx2));
      Point_3 p3(x.at(idx3),y.at(idx3),z.at(idx3));
       Surface_mesh::Vertex_index u = surface_mesh.add_vertex(p1);
       Surface_mesh::Vertex_index v = surface_mesh.add_vertex(p2);
       Surface_mesh::Vertex_index w = surface_mesh.add_vertex(p3);
      surface_mesh.add_face(u, v, w);
