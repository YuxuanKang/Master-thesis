...
int main(int argc, char* argv[])
{
  // Define variables:
     std::vector<vec2 > uvs;
      ...
      Pwn_vector points=loadOBJ("../Straightening_Mesh/data/test.obj",vertexIndices,uvIndices,normalIndices,uvs,out_texture);
      ...
      //----------------------------------Global fitting----------------------------------
       ...
       Efficient_ransac::Shape_range shapes = executeRANSAC(testpoints,0.05,1000,0.02,0.5,0.9);
   ...
   //----------------------Save planes and corresponding points------------------------
       ArrangePlanes(shapes,testpoints,planes,planes_points);
   ...
   //-------------------------------Snap points to planes-------------------------------
       ...
       for(int i=0;i<planes.size();i++)
       {
           snap_points(plane_index,planes.at(plane_index),planes_points.at(i),
                       temp_projected_points,vertex_snapped,plane_vertices,out_map,0.2);
           plane_index++;
       }
      /* //outputVertices("../Straightening_Mesh/data/Snapped_facade.obj",original_points,vertex_snapped);
       ...
       region_segments=RegionGrow(testpoints,planes.size(),vertex_snapped,vertex_region,neighbor_plane,neighbor,out_map);
      ...
      for(int i=0;i<region_segments.size();i++)
      {
          Pwn_vector points_segment= getPointVector(original_points,region_segments.at(i));
          if(points_segment.size()>50)
          {
              Efficient_ransac::Shape_range shapes_segment=executeRANSAC(points_segment,0.1,50,0.05,0.5,0.8);
              std::vector< Pwn_vector > planes_points_segment;
              int plane_num=ArrangePlanes(shapes_segment,points_segment,planes,planes_points_segment);
             ...
             for(int j=0;j<plane_num;j++)
              {
                  snap_points(plane_index,planes.at(plane_index),planes_points_segment.at(j),
                              temp_projected_points,vertex_snapped,plane_vertices,out_map,0.5);
                  plane_index++;
              }
          }
      }
      ...
      std::vector< vector<int> > new_region_segments=RegionGrow(testpoints,planes.size(),vertex_snapped,vertex_region,neighbor_plane,neighbor,out_map,true);
      ...
      writeOBJ("../Straightening_Mesh/data/result_with_spikes.obj",vertexIndices,uvIndices,normalIndices,projected_points,uvs,face_classification,vertex_snapped,vertex_region,out_texture);
      //----------------------------------------------------Remove spikes-------------------------
        for(int i=0;i<new_region_segments.size();i++)
        {
            // if segment is not fitted by any plane
            if(new_region_segments.at(i).size()<30)
            {
                // Snap the points of the segment to its main plane
                 if(neighbor_plane.at(i)!=-1)
                {
                    Pwn_vector point_segment= getPointVector(original_points,new_region_segments.at(i));
                    int plane_id=neighbor_plane.at(i);
                    snap_points(plane_id,planes.at(plane_id),point_segment,temp_projected_points,vertex_snapped,plane_vertices,out_map);
                }
...
*/
    ...
    /*    projected_points.clear();
       ...
       writePlanes("../Straightening_Mesh/data/planes.obj",plane_vertices);
       writeOBJ("../Straightening_Mesh/data/result_remove_spikes.obj",vertexIndices,uvIndices,normalIndices,projected_points,uvs,face_classification,vertex_snapped,vertex_region,out_texture);
       //writeOBJtest("../Straightening_Mesh/data/result.obj",vertexIndices,uvIndices,normalIndices,original_points,uvs,out_texture);
       outputVertices("../Straightening_Mesh/data/Snapped_Vertices.obj",original_points,vertex_snapped);
       return EXIT_SUCCESS;*/
}
